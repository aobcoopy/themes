...
<section class="knsl-mb-30">
	<div class="row">
		<div class="col-lg-12">
			<h3 class="knsl-mb-40"><?php comments_number( esc_html__( 'No reviews found', 'kinsley' ), esc_html__( '1 Review', 'kinsley' ), esc_html__( '% Reviews', 'kinsley' ) ); ?></h3>
		</div>
		...
		<div class="col-lg-12">
			...
			the_comments_navigation( array(
				'screen_reader_text' => ' ',
				'prev_text' => esc_html__( 'Older reviews', 'kinsley' ),
				'next_text' => esc_html__( 'Newer reviews', 'kinsley' )
			) );

			// If comments are closed and there are comments, let's leave a little note, shall we?
			if ( ! comments_open() ) :
				?>
				<p class="no-comments"><?php esc_html_e( 'Reviews are closed.', 'kinsley' ); ?></p>
				<?php
			endif; ?>
...
<section class="form-comment knsl-scroll-animation<?php if ( comments_open() ) : ?> form-comment-open<?php endif; ?>">
		<?php
			...
			$comment_args = array(
				'title_reply' => esc_html__( 'Write a review', 'kinsley' ),
				...
				'comment_field' => '<div class="form-comment-group"><textarea placeholder="' . esc_attr__( 'Review', 'kinsley' ).'" id="comment" name="comment" aria-required="true" rows="6"></textarea></div>',
				'must_log_in' => '<p class="must-log-in">' . esc_html__( 'You must be ', 'kinsley' ) . '<a href="' . esc_url( wp_login_url( apply_filters( 'the_permalink', get_permalink( ) ) ) ) . '">' . esc_html__( 'logged in', 'kinsley' ) . '</a>' . esc_html__( ' to post a review.', 'kinsley' ) . '</p>',
