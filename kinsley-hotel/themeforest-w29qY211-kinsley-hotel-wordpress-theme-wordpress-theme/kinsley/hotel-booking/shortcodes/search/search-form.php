...
<form method="GET" class="mphb_sc_search-form" action="<?php echo esc_attr( $action ); ?>">
	...
	<p class="mphb_sc_search-check-in-date">
		<label for="<?php echo esc_attr( 'mphb_check_in_date-' . $uniqid ); ?>">
			...
			<abbr title="<?php printf( esc_attr_x( 'Formatted as %s', 'Date format tip', 'motopress-hotel-booking' ), MPHB()->settings()->dateTime()->getDateFormatJS() ); ?>">*</abbr>
		</label>
		<br />
		<input
			...
			<label for="<?php echo esc_attr( 'mphb_children-' . $uniqid ); ?>">
				<?php
					...
					if ( empty( $childrenAge ) ) {
						...
						printf( esc_html__( 'Children %s', 'motopress-hotel-booking' ), $childrenAge );
					}
				?>
			</label>
			<br />
	...
	<p class="mphb_sc_search-submit-button-wrapper">
		<button type="submit" class="button knsl-btn"><?php esc_html_e( 'Search', 'motopress-hotel-booking' ); ?></button>
	</p>
